...
class GraphApiClient:
        ...
        try:
            ...
            if self._token is None or (self._token_expiry and datetime.now() >= self._token_expiry):
                token_response = self._credential.get_token(
                    f"{self.resource_url}/.default")
                self._token = token_response.token
                # Set expiry to 5 minutes before actual expiry for safety
                self._token_expiry = datetime.now(
                ) + timedelta(seconds=token_response.expires_on - time.time() - 300)
    ...
    def _make_request(self,
        ...
        if retry_count > max_retries:
            raise GraphApiError(
                f"Max retries ({max_retries}) exceeded for {method} {uri}")
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                if retry_count < max_retries:
                    self._log(
                        f"Rate limited. Retrying after {retry_after} seconds (attempt {retry_count + 1}/{max_retries})")
                    time.sleep(retry_after)
                    return self._make_request(uri, method, data, headers, timeout, max_retries, retry_count + 1)
                else:
                    raise GraphApiError(f"Rate limit exceeded and max retries reached", 429, response.json(
            ...
            if response.status_code == 401:
                if retry_count < max_retries:
                    self._log(
                        f"Authentication failed. Refreshing token and retrying (attempt {retry_count + 1}/{max_retries})")
                    self._token = None
                    self._token_expiry = None
                    return self._make_request(uri, method, data, headers, timeout, max_retries, retry_count + 1)
                else:
                    raise GraphApiError(f"Authentication failed after {max_retries} retries", 401, response.json(
                    ) if response.content else None)
            ...
            if 400 <= response.status_code < 500:
                ...
                raise GraphApiError(
                    f"Client error: {error_message}", response.status_code, error_data)
            ...
            if response.status_code >= 500:
                if retry_count < max_retries:
                    ...
                    self._log(
                        f"Server error {response.status_code}. Retrying after {wait_time} seconds (attempt {retry_count + 1}/{max_retries})")
                    time.sleep(wait_time)
                    return self._make_request(uri, method, data, headers, timeout, max_retries, retry_count + 1)
                else:
                    error_data = response.json() if response.content else {}
                    raise GraphApiError(
                        f"Server error after {max_retries} retries", response.status_code, error_data)
