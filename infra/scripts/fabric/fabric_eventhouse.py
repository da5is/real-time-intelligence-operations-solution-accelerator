...
def _rename_default_database(workspace_client: FabricWorkspaceApiClient,
    ...
    for attempt in range(max_retries):
        try:
            ...
            default_database = workspace_client.get_kql_database_by_name(
                eventhouse_name)
            ...
            if default_database:
                ...
                if current_name == database_name:
                    print(
                        f"ℹ️  Default database already has the desired name '{database_name}'")
                    return True

                # Rename the database
                print(
                    f"🏗️  Renaming default database from '{current_name}' to '{database_name}'...")
                ...
                if attempt < max_retries - 1:
                    print(
                        f"⏳ Default database '{eventhouse_name}' not found yet, retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries})")
                    time.sleep(retry_delay)
                else:
                    print(
                        f"⚠️  Could not find default database '{eventhouse_name}' after {max_retries} attempts")
                    return False

        except FabricApiError as e:
            if attempt < max_retries - 1:
                print(
                    f"⏳ Database rename failed, retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries}): {e}")
                time.sleep(retry_delay)
            else:
                print(
                    f"⚠️  Failed to rename default database after {max_retries} attempts: {e}")
                return False
        ...
        try:
            ...
            if result and isinstance(result, dict):
                ...
                try:
                    ...
                    # Rename the default database if a database name is provided
                    _rename_default_database(
                        workspace_client, eventhouse_name, database_name)
